...
class DB
{
    ...
    /**
	 * ADDED BY IBRAHIM - 23 oct 09 
	 * Builds a join SQL SELECT statement
	 *
	 * @param string $tableName The name of the table
	 * @param array $whereArray (Optional) An associative array containing the
	 *                          column names as keys and values as data. The
	...
	static public function BuildSQLLeftJoinSelect($tableName, $whereArray = null, $columns = null,
										  $sortColumns = null, $sortAscending = true, $limit = null, $join = 'LEFT JOIN') {
		if (! is_null($columns)) {
			$sql = self::BuildSQLColumns($columns, FALSE );
		} else {
			$sql = "*";
		}
		...
		if( is_array( $tableName ) ){
			$tableSql = ' `'.$tableName[0]['name']. '` as '.$tableName[0]['alias'];
			array_shift( $tableName );
			foreach( $tableName as $tN ){
				$tableSql .= ' '.$join.' `'.$tN['name']. '` as '.$tN['alias'].' ON '.$tN['join_left'].'='.$tN['join_right'];	
			}
		} else {
			$tableSql = $tableName;
		}
   ...
   /**
	...
	public function Open($database="", $server="", $username="",
		...
		if ($pcon) {
			$this->mysql_link = @mysqli_pconnect($server, $username, $password, $database);
		} else {
			$this->mysql_link = @mysqli_connect($server, $username, $password,$database);
		}
	 ...
	 *                Values can be: MYSQLI_ASSOC, MYSQL_NUM, MYSQL_BOTH
	...
	public function RecordsArray($resultType=MYSQLI_BOTH) {
	    $this->ResetError();
	    if ($this->last_result) {
	        if( ( mysqli_num_rows ( $this->last_result ) > 0 ) ){
	...
	public function RowArray($optional_row_number = null, $resultType = MYSQLI_BOTH) {
	   ...
	   // $row = mysqli_fetch_array($this->last_result, $resultType);
		$row = mysqli_fetch_assoc($this->last_result);
